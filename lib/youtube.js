...
async function youtubeChannelId(accessToken) {
  const url = new URL(CHANNEL_URL);
  url.search = new URLSearchParams({ part:'id', mine:'true', maxResults:'1' }).toString();
  const data = await youtubeApi(url, accessToken);
  const channelId = data.items?.[0]?.id;
  if (!channelId) throw new AppError('No YouTube channel was found for this Google account.');
  return channelId;
}
async function youtubeVideoTitles(accessToken, ids) {
  const out = {};
  const unique = [...new Set(ids.filter(Boolean))].slice(0,50);
  if (!unique.length) return out;
  const url = new URL('https://www.googleapis.com/youtube/v3/videos');
  url.search = new URLSearchParams({ part:'snippet', id:unique.join(','), maxResults:'50' }).toString();
  const data = await youtubeApi(url, accessToken);
  for (const item of data.items || []) out[item.id] = item.snippet?.title || item.id;
  return out;
}
async function youtubeHandledMap(ids) {
  const out = {};
  for (const commentId of ids) {
    const value = await read('comment-handled:youtube:' + commentId);
    if (value) out[commentId] = value;
  }
  return out;
}
export const youtubeCommentsEndpoint = endpoint(async (req,res) => {
  only(req,res,['GET']);
  await authorize(req);
  const c = await youtubeConnection();
  if (!c) throw new AppError('Connect YouTube first.');
  const channelId = await youtubeChannelId(c.accessToken);
  ...
  for (const thread of data.items || []) {
    ...
    for (const reply of thread.replies?.comments || []) {
      ...
      rows.push({
        ...
        id:reply.id,
        threadId:thread.id,
        parentId:reply.snippet?.parentId || top?.id || null,
        videoId:thread.snippet?.videoId || null,
  ...
  rows.sort((a,b)=>Date.parse(b.publishedAt || 0)-Date.parse(a.publishedAt || 0));
  const titles = await youtubeVideoTitles(c.accessToken, rows.map(x=>x.videoId));
  const handled = await youtubeHandledMap(rows.map(x=>x.id));
  const items = rows.map(row => ({
    ...row,
    postTitle: titles[row.videoId] || 'YouTube video',
    handled: !!handled[row.id],
    handledAt: handled[row.id]?.handledAt || null,
    openUrl: row.videoId ? 'https://www.youtube.com/watch?v=' + encodeURIComponent(row.videoId) + '&lc=' + encodeURIComponent(row.id) : 'https://www.youtube.com/'
  }));
  res.json({ items, checkedAt:now() });
});
export const youtubeCommentReplyEndpoint = endpoint(async (req,res) => {
  only(req,res,['POST']);
  const session = await authorize(req);
  const b = input(req);
  const parentId = cleanVideoText(b.parentId || '',100,'Parent comment ID');
  const textOriginal = cleanVideoText(b.text || '',10000,'Reply');
  if (!parentId) throw new AppError('Choose a comment to reply to.');
  if (!textOriginal) throw new AppError('Write a reply first.');
  const c = await youtubeConnection();
  if (!c) throw new AppError('Connect YouTube first.');
  const data = await youtubeWrite('https://www.googleapis.com/youtube/v3/comments?part=snippet',c.accessToken,'POST',{
    snippet:{parentId,textOriginal}
  });
  await write('comment-handled:youtube:' + parentId,{handledAt:now(),handledBy:session.name,reason:'replied'});
  res.json({success:true,replyId:data?.id || null,handledAt:now()});
});
export const youtubeCommentHandledEndpoint = endpoint(async (req,res) => {
  only(req,res,['POST']);
  const session = await authorize(req);
  const b = input(req);
  const commentId = cleanVideoText(b.commentId || '',100,'Comment ID');
  if (!commentId) throw new AppError('Choose a comment.');
  const handled = b.handled !== false;
  if (handled) await write('comment-handled:youtube:' + commentId,{handledAt:now(),handledBy:session.name,reason:'manual'});
  else await redis('DEL', PREFIX + 'comment-handled:youtube:' + commentId);
