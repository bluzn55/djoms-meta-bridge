...
async function validateRecord(record, online = false) {
  ...
  if (online) {
    ...
    for (const target of record.targets) if (!status[target]?.canPublish) throw new AppError(status[target]?.message || 'Check the account connection.');
    return status;
  }
}
...
export function publicRecord(r) { return { ...r, imageUrl: r.imageId ? mediaUrl(r.imageId) : null }; }
export const recordsEndpoint = endpoint(async (req, res) => {
  ...
  const result = await withLock('record:' + b.id, async () => {
    ...
    if (b.action === 'duplicate') {
      ...
      copy.caption = r.caption; copy.link = r.link; copy.imageId = r.imageId; copy.targets = [...r.targets];
    ...
    if (b.action === 'save') {
      ...
      if (!Array.isArray(b.targets) || b.targets.some(t => !['facebook','instagram'].includes(t))) throw new AppError('Choose Facebook or Instagram.');
...
function summarize(r) {
  const states = r.targets.map(t => r.results[t]?.status || 'pending');
...
export async function publishRecord(recordId, user, expectedRevision, scheduleToken) {
  return withLock('record:' + recordId, async () => {
    ...
    for (const platform of r.targets) await sendPlatform(r, platform, c, status);
...
export const mediaEndpoint = endpoint(async (req, res) => {
  ...
  if (typeof b.base64 !== 'string' || b.base64.length > 1400000 || !/^[A-Za-z0-9+/]+={0,2}$/.test(b.base64)) throw new AppError('Choose a photo that can be fitted below 1 MB.');
  const bytes = Buffer.from(b.base64, 'base64'), size = jpegSize(bytes);
  if (!size || bytes.length > 1048576 || size.width !== size.height || size.width < 320 || size.width > 1080) throw new AppError('Upload a square JPEG photo between 320 and 1080 pixels.');
