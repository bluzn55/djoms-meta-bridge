...
export function publicRecord(r) {
  ...
  return { ...r, imageUrl: r.imageId ? mediaUrl(r.imageId) : null, imageVariants };
}
...
export async function publishRecord(recordId, user, expectedRevision, scheduleToken) {
  return withLock('record:' + recordId, async () => {
    ...
    for (const platform of r.targets.filter(p => !['facebook','instagram'].includes(p))) {
      r.results[platform] = { status:'failed', error: platform === 'x' ? 'X campaign version is prepared, but image publishing is not enabled yet.' : 'YouTube campaign version is prepared for video/Shorts production; static-image publishing is not used.' };
    }
